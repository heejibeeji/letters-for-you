...
import { getFirestore, collection, addDoc, deleteDoc, doc, onSnapshot, query, orderBy } from "https://www.gstatic.com/firebasejs/12.10.0/firebase-firestore.js";
...
// sort newest first
allLetters.sort((a, b) => new Date(b.date) - new Date(a.date));
...
// ---------- HOME VIEW ----------
function showHome(list = allLetters, page = 1) {
  currentPage = page;
  content.innerHTML = "";
...
function addPagination(totalLetters) {
  ...
  for (let i = 1; i <= totalPages; i++) {
    ...
    btn.onclick = () => showHome(allLetters, i);
    nav.appendChild(btn);
  }
...
function showFull(letter) {
  ...
  content.querySelector(".back").onclick = () => {
    ...
    showHome(allLetters, currentPage);
  };
}
...
// Load Firebase letters in real-time
try {
  const q = query(collection(db, "letters"), orderBy("createdAt", "desc"));

  onSnapshot(q, (snapshot) => {
    const firebaseLetters = [];
    ...
    // Combine and sort
    allLetters = [...letters, ...firebaseLetters];
    allLetters.sort((a, b) => {
      const timeA = a.firebaseId ? (a.createdAt || 0) : new Date(a.date).getTime();
      const timeB = b.firebaseId ? (b.createdAt || 0) : new Date(b.date).getTime();
      return timeB - timeA;
    });
